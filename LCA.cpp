...
void finLcaBt(TreeNode* root,int a,int b)
{
	if (root == NULL) return;
	
	finLcaBt(root->left,int a,int b);
	if(aPresent && bPresent) return; //lca was there is left subtree.
	...
	finLcaBt(root->right,int a,int b);
...
int main() {
    ...
    int a, b; cin >> a >> b;
    TreeNode* lca = finLcaBt(root,a,b);
